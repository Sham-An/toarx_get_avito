...
#
# Любые словари, переданные методу запроса? будут объединены с заданными значениями уровня сессии.
# Параметры уровня методов перезаписывают параметры сессии.
#
# Удалите значение из параметра словаря:
# Иногда нужно будет не включать ключи уровня сессии в параметры словаря.
# Для этого необходимо установить значения ключа None в параметре уровня методов.
# Они будут пропускаться автоматически.
#
...
# Если точнее, состояние уровня Session, например куки, не будет применено к запросу.
# Чтобы получить PreparedRequest с примененным состоянием, замените вызов к
# Request.prepare() вызовом к Session.prepare_request():
#
# from requests import Request, Session
#
# s = Session()
