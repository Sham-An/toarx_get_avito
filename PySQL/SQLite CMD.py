...
# Эта строка создает объект connection,
# а также новый файл orders.db в рабочей директории.
# Если нужно использовать другую, ее нужно обозначить явно:

# conn = sqlite3.connect(r'PySQL/orders.db')
conn = sqlite3.connect('orders.db')


# перед строкой с путем стоит символ «r». Это дает понять Python, что речь идет о «сырой» строке,
# где символы «/» не отвечают за экранирование.
...
def create_table1():
    # conn1 = sqlite3.connect(r'PySQL/orders.db')
    # conn1 = sqlite3.connect('orders.db')
    cur1 = conn.cursor()
...
# Для создания второй таблицы просто повторим последовательность действий,
# используя следующие команды:

def create_table2():
    # cur3 = cur1
    cur3 = conn.cursor()
    cur3.execute("""CREATE TABLE IF NOT EXISTS orders(
    ...
    # cur3.execute("""INSERT INTO users(userid, fname, lname, gender)
    ...
    #
    # cur3.executemany("INSERT INTO users VALUES(?, ?, ?, ?);", more_users)
    # conn.commit()
    #
...
#     cur3.executemany("INSERT INTO users VALUES(?, ?, ?, ?);", customers)
#     cur3.executemany("INSERT INTO orders VALUES(?, ?, ?, ?);", orders)
#     conn.commit()
#
def select_from_table():
# #Создадим переменную one_result для получения только одного результата:
    cur3 = conn.cursor()
    cur3.execute("SELECT * FROM users;")
    one_result = cur3.fetchone()
    print(one_result)
#
# #Если же нужно получить много данных, то используется функция fetchmany(). Выполним другой скрипт для генерации 3 результатов:
#
def select_from_table2():
    cur3 = conn.cursor()
    cur3.execute("SELECT * FROM users;")
    three_results = cur3.fetchmany(3)
    print(three_results)
#
def select_from_table_all():
    cur3 = conn.cursor()
# #Функцию fetchall() можно использовать для получения всех результатов. Вот что будет, если запустить скрипт:
    cur3.execute("SELECT * FROM users;")
    all_results = cur3.fetchall()
    print(all_results)
#
